...
async def scrape_store(url: str) -> StoreInfo:
    ...
    try:
        ...
        if response.status_code == 200:
            name, emails, phones, socials, contacts, addresses, prices, products = clean_and_extract(response.text, url)
    ...
    if all_emails and all_phones:
        return StoreInfo(name=store_name, emails=list(all_emails), phones=list(all_phones), social_links=all_socials, addresses=all_addresses, prices=all_prices, products=all_products, url=url)
